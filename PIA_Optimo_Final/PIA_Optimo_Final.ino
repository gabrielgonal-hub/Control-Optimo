...
void setup(){
  ...
  Kp_1 = K1_1 + K2_1 + 1 + (K1_1*R2+R1+R2)/R3;
  ...
  // SISTEMA 3 (LQR)
  // =====================================================
  Kp_3 = K1_3 + K2_3 + 1 + (K1_3*R2+R1+R2)/R3;
...
void loop(){
  ...
  // ===== SISTEMA 1 =====
  Y1 = analogRead(X2_1)*mX;
  float e1 = Y1 - Xe2_1;

  float f1_1 = Am11*Xe1_1 + Am12*Xe2_1 + Bm1*U1 + H1_1*e1;
  float f2_1 = Am21*Xe1_1 + Am22*Xe2_1 + H2_1*e1;
  ...
  // ===== SISTEMA 2 =====
  Y2 = analogRead(X2_2)*mX;
  float e2 = Y2 - Xe2_2;

  float f1_2 = Am11*Xe1_2 + Am12*Xe2_2 + Bm1*U2 + H1_2*e2;
  float f2_2 = Am21*Xe1_2 + Am22*Xe2_2 + H2_2*e2;
  ...
  // ===== SISTEMA 3 =====
  float X13 = analogRead(X1_3)*mX;
  float X23 = analogRead(X2_3)*mX;
  Y3 = X23;

  U3 = Kp_3*R - K1_3*X13 - K2_3*X23;

  // ===== SISTEMA 4 =====
  float X14 = analogRead(X1_4)*mX;
  float X24 = analogRead(X2_4)*mX;
  Y4 = X24;
  ...
  // ===== SATURACIÓN =====
  U1 = constrain(U1,0,3.3);
  U2 = constrain(U2,0,3.3);
  U3 = constrain(U3,0,3.3);
  U4 = constrain(U4,0,3.3);
...
void coms_arduino_ide(){
    ...
    // Serial.print(" | Y1:"); Serial.print(Y1); Serial.print(" U1:"); Serial.print(U1);
    Serial.print(" | Y2:"); Serial.print(Y2); Serial.print(" U2:"); Serial.print(U2);
    Serial.print(" | Y3:"); Serial.print(Y3); Serial.print(" U3:"); Serial.print(U3);
    Serial.print(" | Y4:"); Serial.print(Y4); Serial.print(" U4:"); Serial.println(U4);
}
                 ...
                 float* X14,float* Y4p, float* U4p)
{
  ...
  byte buf[56] = {
    ...
    X1B1[0],X1B1[1],X1B1[2],X1B1[3],
    Y1B[0],Y1B[1],Y1B[2],Y1B[3],
    U1B[0],U1B[1],U1B[2],U1B[3],

    X1B2[0],X1B2[1],X1B2[2],X1B2[3],
    Y2B[0],Y2B[1],Y2B[2],Y2B[3],
    U2B[0],U2B[1],U2B[2],U2B[3],

    X1B3[0],X1B3[1],X1B3[2],X1B3[3],
    Y3B[0],Y3B[1],Y3B[2],Y3B[3],
    U3B[0],U3B[1],U3B[2],U3B[3],

    X1B4[0],X1B4[1],X1B4[2],X1B4[3],
    Y4B[0],Y4B[1],Y4B[2],Y4B[3],
    U4B[0],U4B[1],U4B[2],U4B[3]
  };
